module.exports = (express) => {
	...
	router.route(['/tags', '/tags/:permission']).get((req, res) => {
		...
		mDesign.find(query,['signhash', 'tags', 'publish'], function(err, designs) {
			if(err) res.jsonp({ code: 418, service: 'design', function: 'tags', message: 'error', error: err});
			const tagList = {};
			...
			setTimeout(()=> res.jsonp({ code: 410, service: 'design', function: 'tags', message: 'success', tags: tagList, signhash: myhash}),500);
		});
	}).all((req, res) => res.jsonp({code: 419, service: 'design', function: 'tags', message: 'unauthorized_method'}));
